...
import { getVotingTopicById, updateVotingTopicById } from 'apiSdk/voting-topics';
...
function VotingTopicEditPage() {
  ...
  const handleSubmit = async (values: VotingTopicInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateVotingTopicById(id, values);
      mutate(updated);
      resetForm();
